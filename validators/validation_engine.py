...
from config.settings import NodeLabel, RelType, Severity, OUTPUT_DIR, MARGINAL_TOLERANCE
...
from extractors.rule_extractor import infer_operator
...
# Gradient/ratio like "1:100", "1 in 200", "1in200" → slope fraction (0.01, 0.005)
_RATIO_RE = re.compile(r"(\d+(?:\.\d+)?)\s*(?::|in)\s*(\d+(?:\.\d+)?)", re.I)
_GRAD_KINDS = {"ratio", "percent", "permille"}


def _parse_quantity(s) -> tuple[Optional[float], Optional[str]]:
    """
    Parse a value string into (number, kind).

    kind ∈ {'ratio','percent','permille','plain', None}. This replaces the old
    digit-stripping _extract_num which mangled "1:100"→1100 and returned None for
    any string with two numbers (e.g. "7.85 m ... 13.15 m"), forcing false fails.
    """
    if s is None:
        return (None, None)
    t = str(s).strip().lower()
    ...
    if m and float(m.group(2)) != 0:
        return (float(m.group(1)) / float(m.group(2)), "ratio")
    m = re.search(r"(-?\d+(?:\.\d+)?)\s*(?:%|percent)", t)
    if m:
        return (float(m.group(1)), "percent")
    m = re.search(r"(-?\d+(?:\.\d+)?)\s*(?:mm\s*/\s*m|‰)", t)
    if m:
        return (float(m.group(1)), "permille")
    m = re.search(r"-?\d+(?:\.\d+)?", t)   # first plain number (ignores trailing prose)
    if m:
        return (float(m.group(0)), "plain")
    return (None, None)


def _to_slope(v: float, kind: str) -> float:
    """Normalise a gradient-ish value to a slope fraction for comparison."""
    if kind == "percent":
        return v / 100.0
    if kind == "permille":
        return v / 1000.0
    return v  # 'ratio' is already a slope; 'plain' is assumed to be on the same scale


def _is_ambiguous_threshold(threshold) -> bool:
    """
    True when a threshold packs several condition-specific values into one string
    (e.g. "7.85 m single line; 13.15 m double line", or a multi-clause ruling
    gradient). These cannot be compared deterministically — they are routed to
    'Needs Review' for the adjudicator/human instead of being failed outright.
    """
    t = str(threshold).lower()
    nums = re.findall(r"\d+(?:\.\d+)?", t)
    if len(nums) < 2:
        return False
    markers = (";", "single", "double", "3rd", "4th", "degree", "kmph",
               "approach", "station", "yard", " or ")
    return any(m in t for m in markers)


def _string_match(fact_val: str, threshold: str, operator: str) -> Optional[bool]:
    """Fallback non-numeric comparison for == / must_be."""
    fv, tv = str(fact_val).strip().lower(), str(threshold).strip().lower()
    if operator == "==":
        return fv == tv
    if operator == "must_be":
        return tv in fv
...
def _extract_num(s) -> Optional[float]:
    """Back-compat shim — returns just the numeric part (kind discarded)."""
    return _parse_quantity(s)[0]


def _compare(fact_val: str, operator: str, threshold: str) -> Optional[bool]:
    """
    Apply operator between fact value and rule threshold.
    Returns True (compliant), False (non-compliant), None (cannot compare → review).

    Handles gradient/ratio/percent/per-mille units by normalising both sides to a
    common slope before comparing, so e.g. DPR "0.142 %" vs rule "1:100" is judged
    correctly (0.00142 ≤ 0.01 → compliant) instead of being failed on raw numbers.
    """
    fv, fk = _parse_quantity(fact_val)
    tv, tk = _parse_quantity(threshold)

    if fv is None or tv is None:
        return _string_match(fact_val, threshold, operator)

    # Reconcile gradient-style units to a common slope scale before comparing.
    ...
    ops = {
        ...
        "<":  fv < tv,
        "==": abs(fv - tv) < 1e-6,
        # Loose verbs: treat as presence/equality of the numeric value.
        "must_be":  abs(fv - tv) < 1e-6,
        "requires": abs(fv - tv) < 1e-6,
    }
    if operator in ops:
        return ops[operator]

    if operator == "in_range":
        parts = re.findall(r"\d+(?:\.\d+)?", str(threshold))
        if len(parts) >= 2:
...
# Map a comparison status to a report classification.
_STATUS_TO_CLASS = {
    "pass":     "Compliant",
    "marginal": "Marginal",
    "fail":     "Non-Compliant",
...
def _compare_status(fact_val: str, operator: str, threshold: str) -> str:
    """
    Tolerance-aware comparison. Returns one of:
      'pass'     — meets the requirement
      'marginal' — fails but within MARGINAL_TOLERANCE of the bound (near-miss)
      'fail'     — clearly violates
      'review'   — cannot be compared (defer to adjudicator / human)
    """
    fv, fk = _parse_quantity(fact_val)
    tv, tk = _parse_quantity(threshold)

    if fv is None or tv is None:
        sm = _string_match(fact_val, threshold, operator)
        if sm is None:
            return "review"
    ...
    if operator in (">=", ">"):
        if fv >= tv:        return "pass"
        if fv >= tv - tol:  return "marginal"
        return "fail"
    if operator in ("<=", "<"):
        if fv <= tv:        return "pass"
        if fv <= tv + tol:  return "marginal"
        return "fail"
    if operator in ("==", "must_be", "requires"):
        return "pass" if abs(fv - tv) <= tol else "fail"
    if operator == "in_range":
        parts = re.findall(r"\d+(?:\.\d+)?", str(threshold))
        if len(parts) >= 2:
            lo, hi = sorted((float(parts[0]), float(parts[1])))
            if lo <= fv <= hi:               return "pass"
            if lo - tol <= fv <= hi + tol:   return "marginal"
            return "fail"
    return "review"
...
def _match_facts_to_rules(doc_id: str, sector: str) -> list[ValidationRow]:
    ...
    for p in pairs:
        ...
        # often a wrong "==" for minimum/ruling rules (see rule_extractor.infer_operator).
        eff_op = infer_operator(str(p.get("rtext", "")), str(p.get("rattr", "")), str(p.get("rop", "")))
        ...
        if status == "pass":
            ...
            reason = (
                f"The DPR reports {p['fattr']} as {dpr_display}, marginally off the "
                f"requirement of {rule_display} (within {MARGINAL_TOLERANCE:.0%} tolerance) "
                f"per {p['std']} {p['clause']}. Acceptable but worth confirming."
            )
        elif status == "fail":
            reason = (
                f"The DPR reports {p['fattr']} as {dpr_display}, "
            ...
            # Cannot be judged deterministically — flag for the adjudicator/human,
            # do NOT count it as a failure (this was the old behaviour that tanked
            # the score with un-parseable thresholds).
            reason = (
                f"The DPR value for {p['fattr']} is '{dpr_display}'. "
                f"The rule requires {rule_display} per {p['std']} {p['clause']}. "
                f"This could not be verified automatically and needs review."
            )
        ...
        rows.append(ValidationRow(
            ...
            severity=severity,
            reason=reason,
            # Prefer the printed DPR page number; fall back to the physical PDF page.
            source_page=int(p["fppage"] if p.get("fppage") else (p["fpage"] or 0)),
            weight=weight,
            fact_id=str(p["fid"]),
            rule_id=str(p["rid"]),
            doc_id=doc_id,
    ...
    if unmatched_rules:
        ...
        all_facts = run_read(
            ...
            RETURN f.fact_id AS fid, f.attribute AS fattr, f.subject AS fsubj,
                   ...
                   f.printed_page AS fppage, f.confidence AS fconf
            LIMIT 2000
            """,
            {"doc_id": doc_id}
        )
            ...
            for r_idx, rule in enumerate(unmatched_rules):
                ...
                if status == "pass":
                    reason = (
                        ...
                        f"{rule.get('std','')} {rule.get('clause','')}. {sem_note}"
                    )
                elif status == "marginal":
                    reason = (
                        f"The DPR reports {fact.get('fattr','')} as {dpr_display}, marginally off "
                        f"{rule_display} (within {MARGINAL_TOLERANCE:.0%}) per "
                        f"{rule.get('std','')} {rule.get('clause','')}. {sem_note}"
                    )
                elif status == "fail":
                    reason = (
                    ...
                    reason = (
                        ...
                        f"(similarity={best_sim:.2f} to rule attribute '{rule.get('rattr','')}'), "
                        f"but the value could not be automatically compared. "
                        f"Manual review needed for {rule.get('std','')} {rule.get('clause','')}."
                    )
                ...
                rows.append(ValidationRow(
                    ...
                    severity=severity,
                    reason=reason,
                    source_page=int(fact.get("fppage") if fact.get("fppage") else (fact.get("fpage") or 0)),
                    weight=weight,
    ...
    for r in rows:
        run_write(
            ...
            """,
            {
                ...
                "status":       {"Compliant": "PASS", "Marginal": "MARGINAL", "Non-Compliant": "FAIL"}.get(r.classification, "REVIEW"),
                "severity":     r.severity,
...
def _compute_score(rows: list[ValidationRow]) -> dict:
    ...
    Score = Σ(weights of Compliant rows) / Σ(weights of JUDGED rows) × 100
    where JUDGED = Compliant + Non-Compliant. "Needs Review" rows are excluded
    from the denominator so un-verifiable checks don't drag the score down.
    ...
    # "Marginal" (near-miss within tolerance) counts as compliant credit for the
    # score but is tracked and reported separately so it stays visible.
    _PASS = ("Compliant", "Marginal")
    judged            = [r for r in rows if r.classification in ("Compliant", "Marginal", "Non-Compliant")]
    needs_review      = [r for r in rows if r.classification == "Needs Review"]
    marginal          = [r for r in rows if r.classification == "Marginal"]
    total_weight      = sum(r.weight for r in judged)
    compliant_weight  = sum(r.weight for r in judged if r.classification in _PASS)
    ...
    # Verdict band — only meaningful if something was actually judged
    verdict = "NO_CHECKS_RUN" if not judged else "POOR"
    for threshold, label in VERDICT_BANDS:
        if judged and weighted_score >= threshold:
            verdict = label
            break
    ...
    for sev in (Severity.CRITICAL, Severity.HIGH, Severity.MEDIUM, Severity.LOW):
        ...
        if sev_rows:
            ...
            sev_marginal  = sum(1 for r in sev_rows if r.classification == "Marginal")
            sev_nc        = sum(1 for r in sev_rows if r.classification == "Non-Compliant")
            sev_review    = sum(1 for r in sev_rows if r.classification == "Needs Review")
            by_severity[sev] = {
                "total":            len(sev_rows),
                "compliant":        sev_compliant,
                "marginal":         sev_marginal,
                "non_compliant":    sev_nc,
                "needs_review":     sev_review,
                "weight":           sum(r.weight for r in sev_rows if r.classification in ("Compliant", "Marginal", "Non-Compliant")),
                "compliant_weight": sum(r.weight for r in sev_rows if r.classification in ("Compliant", "Marginal")),
            }

    # Breakdown by category (rulebook section / standard)
    from collections import defaultdict
    cat_data: dict[str, dict] = defaultdict(lambda: {"total": 0, "compliant": 0, "marginal": 0, "non_compliant": 0, "needs_review": 0})
    for r in rows:
        ...
        if r.classification == "Compliant":
            ...
            cat_data[cat]["marginal"] += 1
        elif r.classification == "Non-Compliant":
    ...
    for cat, data in cat_data.items():
        passing    = data["compliant"] + data["marginal"]   # marginal = within tolerance
        judged_cat = passing + data["non_compliant"]         # exclude needs_review
        by_category[cat] = {
            "total":         data["total"],
            ...
            "category_score": round(passing / judged_cat * 100, 1) if judged_cat else None,
        }
...
def _build_report(
    ...
    # Sort: Non-Compliant first (CRITICAL → LOW), then Marginal, Needs Review, Compliant
    sev_order = {Severity.CRITICAL: 0, Severity.HIGH: 1, Severity.MEDIUM: 2, Severity.LOW: 3, Severity.INFO: 4}
    _by_sev = lambda group: sorted(group, key=lambda r: sev_order.get(r.severity, 5))
    sorted_rows = _by_sev(non_compliant) + _by_sev(marginal) + _by_sev(needs_review) + _by_sev(compliant)
