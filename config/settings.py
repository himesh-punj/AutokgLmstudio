...
NEO4J_USER     = "neo4j"
NEO4J_PASSWORD = "root1234"   # ← CHANGE THIS to your Neo4j password
...
TABLE_MIN_FILL = 0.75   # pdfplumber/camelot table acceptance threshold (0-1).
                        ...
                        # accepted, else fall through (camelot → vision LLM). Vision is
                        # the accuracy backstop and is NOT skipped — speed comes from
                        # not re-parsing the PDF per page (see table_extractor thread cache).
...
# Near-miss tolerance for compliance checks. A value that fails a >=/<=/== bound
# but lands within this relative band of the threshold is classed "Marginal"
# (counts as compliant for scoring, but is flagged for attention) instead of a
# hard Non-Compliant. e.g. 0.02 → formation width 7.8 m vs required 7.85 m
# (0.6% under) is Marginal, not a failure; 5.3 m vs 7.85 m stays Non-Compliant.
MARGINAL_TOLERANCE = 0.02   # 2%
