...
def push_facts(facts: list[dict], sector: str, dry_run: bool) -> int:
    if dry_run:
        return len(facts)
    pushed = 0
    for fact in facts:
        fact_id = str(uuid.uuid4())
        # Fact dicts from extraction carry no sector — use the document's sector so
        # the Fact->Sector BELONGS_TO link is created. Validation's pass-1 join
        # requires it; an empty sector means zero facts ever match any rule.
        fact_sector = fact.get("sector") or sector
        run_write(
            f"""
            ...
            """,
            {
                ...
                "nature":      fact.get("nature", "proposed"),
                "sector":      fact_sector,
...
def push_rules(rules: list[dict], sector: str, standard_name: str, dry_run: bool) -> int:
    if dry_run:
        return len(rules)
    pushed = 0
    for rule in rules:
        # Rule dicts from extraction carry no sector/standard_name — use the
        # rulebook's values so the Rule->Sector BELONGS_TO link is created
        # (validation queries rules via that relationship; an empty sector
        # silently drops every rule) and the report's Standard column is correct.
        rule_sector = rule.get("sector") or sector
        rule_std    = rule.get("standard_name") or standard_name
        run_write(
            f"""
...
def main():
        ...
        for rb_id in rulebook_ids:
            try:
                ...
                if rules_raw and not args.dry_run:
                    ...
                    pushed = push_rules(rules_raw, sector_rb, std_name, args.dry_run)
    ...
    if doc_id:
        ...
        if not args.dry_run:
            ...
            pushed_facts = push_facts(facts_norm, sector, args.dry_run)
