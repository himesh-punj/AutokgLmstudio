...
CHOOSING THE OPERATOR (this is critical — most rules are limits, NOT exact values):
- "minimum", "at least", "not less than", "shall be ≥"      → ">="   (e.g. minimum formation width 7.85 m)
- "maximum", "ruling", "shall not exceed", "not more than",
  "up to", "limited to"                                       → "<="   (e.g. ruling gradient 1:100 — flatter is fine)
- a true single exact value with no min/max wording           → "=="   (e.g. design speed 160 kmph)
- "between X and Y", "from X to Y"                             → "in_range" (put "X-Y" in threshold)
- Do NOT use "==" for minimum/maximum/ruling rules — that wrongly fails any value that isn't exactly equal.

WRITING THE THRESHOLD:
- Put ONE numeric value (and its unit in "unit"). e.g. threshold="7.85", unit="m".
- If the standard gives different values per condition (single vs double line, by speed, etc.),
  emit a SEPARATE rule object for each, using "condition" to say when it applies.
  e.g. {{"attribute":"formation width", "operator":">=", "threshold":"7.85", "unit":"m", "condition":"single line"}}
       {{"attribute":"formation width", "operator":">=", "threshold":"13.15", "unit":"m", "condition":"double line"}}
- Do not cram multiple numbers + prose into one threshold string.
...
# ─── Operator inference ───────────────────────────────────────────────────────
# The LLM frequently labels minimum/maximum/ruling rules as "==", which makes
# validation reject every value that isn't exactly equal. Re-derive the operator
# from the rule's natural-language phrasing — this is the single biggest source of
# false Non-Compliant findings. Shared with validators.validation_engine so that
# already-stored rules benefit too (it re-infers at compare time).

_MIN_PAT = re.compile(
    r"\b(minimum|min\.?|at least|not less than|no less than|greater than or equal|≥)\b", re.I
)
_MAX_PAT = re.compile(
    r"\b(maximum|max\.?|ruling|shall not exceed|not exceed|not more than|no more than|"
    r"up to|at most|limited to|less than or equal|≤)\b", re.I
)
_RANGE_PAT = re.compile(r"\b(between|in the range|ranges? from)\b", re.I)


def infer_operator(rule_text: str, attribute: str = "", given: str = "") -> str:
    """
    Derive the comparison operator from a rule's phrasing.

    'minimum/at least'           -> '>='
    'maximum/ruling/not exceed'  -> '<='
    'between X and Y'            -> 'in_range'
    otherwise keep an already-directional `given` operator, else fall back to the
    given value or 'must_be'. (A bare '==' from the LLM is treated as weak and only
    kept when no min/max wording is present.)
    """
    blob = f"{attribute} {rule_text}".lower()
    if _RANGE_PAT.search(blob):
        return "in_range"
    is_min = bool(_MIN_PAT.search(blob))
    is_max = bool(_MAX_PAT.search(blob))
    if is_min and not is_max:
        return ">="
    if is_max and not is_min:
        return "<="
    if given in (">=", "<=", ">", "<", "in_range"):
        return given
    return given or "must_be"
...
def extract_rules_from_text(
    ...
    # Re-derive the operator from phrasing — the LLM is unreliable here.
    for r in rules:
        r["operator"] = infer_operator(
            str(r.get("rule_text", "")), str(r.get("attribute", "")), str(r.get("operator", ""))
        )
...
def detect_standard_name(text_sample: str) -> str:
    ...
    from utils.llm_router import generate
